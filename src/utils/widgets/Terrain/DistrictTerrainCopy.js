...
class DistrictTerrain {
    ...
    createGeometry(data) {
        ...
        filterdMeshIndex = filterdMeshIndex.filter(item => {
            ...
            let flag = true;
            if(((item[0] > item[1] && item[1] > item[2]) || (item[1] > item[2] && item[2] > item[0]) || (item[2] > item[0] && item[0] > item[1]))&& minItem > this.gridPointLength) {  
                flag = false;
            }
            return flag;
        })
        filterdMeshIndex.forEach(points => {//原来的顺序生成的三角面片法向量朝向屏幕里的，逆过来改成朝向屏幕外
            let tmp = points[1];
            points[1] = points[2];
            points[2] = tmp;
        })

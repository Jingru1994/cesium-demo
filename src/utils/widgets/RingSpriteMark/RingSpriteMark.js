...
class RingSpriteMark {
    constructor(params) {
        ...
        group.add(spriteLabel, pole, bottomShpere, bottomCircle, bottomGradientRing1.mesh, bottomGradientRing2.mesh, bottomSpreadRing.mesh)
    ...
    createLabel(options) {
        ...
        canvas.width = drawOptions.hGap*2 + drawOptions.fontSize*(cLength + eCLength/1.15 + eLLength/1.5 + nLength/1.7);
        ...
        const flowLineRoute = [{x:lineWidth,y:lineWidth},{x:canvas.width-lineWidth,y:lineWidth},{x:canvas.width-lineWidth,y:canvas.height-lineWidth},{x:lineWidth,y:canvas.height-lineWidth}];
    ...
    createBottomShpere(options) {
        const shpereGeometry = new THREE.SphereGeometry(options.radius,32,16,0,Math.PI*2,0,Math.PI/2);

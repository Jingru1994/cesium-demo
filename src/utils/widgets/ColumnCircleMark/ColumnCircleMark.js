...
class ColumnCircleMark {
    ...
    createRippleCircle(options) {
        ...
        let width1 = radius/15, width2 = radius/15*0.7, width3 = radius/10*0.3
        let interval = (radius - width1 - width2 - width3)/3

        
        const geometry1 = new THREE.RingGeometry( interval+width1, interval, 32 )
        const geometry2 = new THREE.RingGeometry( interval*2+width1+width2, interval*2+width1, 32 )
        const geometry3 = new THREE.RingGeometry( radius, radius-width3, 32 )
        const material1 = new THREE.MeshBasicMaterial( { color: color, side: THREE.DoubleSide, transparent:true, opacity: 0.8 } )
        const material2 = new THREE.MeshBasicMaterial( { color: color, side: THREE.DoubleSide, transparent:true, opacity: 0.4 } )
        const material3 = new THREE.MeshBasicMaterial( { color: color, side: THREE.DoubleSide, transparent:true, opacity: 0.1 } )
    ...
    rippleCircleOpacityChange() {
        this.step += 0.03
        this.rippleCircle.children[0].material.opacity = Math.abs(Math.sin(0.8+this.step))*0.8
        this.rippleCircle.children[1].material.opacity = Math.abs(Math.sin(0.4+this.step))*0.8
        this.rippleCircle.children[2].material.opacity = Math.abs(Math.sin(0.0+this.step))*0.8
    ...
    createText(options) {
        ...
        fontLoad.load(
            'threeFonts/fashionBold_Regular.json',
            response => {
                loadedFont = response
                let textOptions = {
                    font: loadedFont,
                ...
                let material2 = new THREE.MeshBasicMaterial({color: new THREE.Color('rgb(220,220,220)')})
                let mesh = new THREE.Mesh(textGeometry,[material1, material2])
                
                mesh.rotation.x = Math.PI / 2
                textGeometry.computeBoundingBox()
                let offset = -(textGeometry.boundingBox.max.x - textGeometry.boundingBox.min.x) / 2

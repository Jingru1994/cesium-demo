...
export default class TranslationController {
    ...
    add(model) {
        // this.destroy()
        this.model = model
        this.position = Matrix4.getTranslation(
            model.modelMatrix,
            new Cartesian3()
        )
        ...
        this._addSphereAxis()
        // // 添加辅助球
        this._addAuxiliaryBall(this.model.boundingSphere.radius * 2, Color.RED.withAlpha(0.2))
    ...
    // 添加监听器
    _addListener () {
        this.handler.setInputAction(this._clickListener,ScreenSpaceEventType.LEFT_DOWN)
        this.handler.setInputAction(this._clickUpListener,ScreenSpaceEventType.LEFT_UP)
        this.handler.setInputAction(this._moveListener,ScreenSpaceEventType.MOUSE_MOVE)
    }
    ...
    _precessRotation (e, axis) {
        ...
        const angle = CesiumMath.toDegrees(Cartesian3.angleBetween(cartesian, axis.direction))
    ...
    translation (moveVector, unit, moveLength) {
        ...
        this.axisX.translation(moveVector, Matrix3.multiplyByVector(mX, unit, new Cartesian3()), moveLength)
        this.axisY.translation(moveVector, Matrix3.multiplyByVector(mY, unit, new Cartesian3()), moveLength)
        ...
        this.axisSphereX.translation(moveVector, Matrix3.multiplyByVector(mX, unit, new Cartesian3()), moveLength)
        this.axisSphereY.translation(moveVector, Matrix3.multiplyByVector(mY, unit, new Cartesian3()), moveLength)
    ...
    _createSphereAxis () {
        ...
        this.axisSphereZ = new AxisSphere('axisSphereZ', radius, this.position, Color.RED)
        this.axisSphereX = new AxisSphere('axisSphereX', radius, this.position, Color.GREEN)
        this.axisSphereY = new AxisSphere('axisSphereY', radius, this.position, Color.BLUE)
    ...
    _rotateVectorByAxisForAngle(vector, axis, angle){
        const rotateQuaternion = this.normalizingQuaternion(Quaternion.fromAxisAngle(axis, angle, new Quaternion()))
    ...
    getPlanePosition (position, cameraPosition, pickRay, axisDirection) {
        // 第一步, 获取相机在轴上的投影
        const cartesian3 = Cartesian3.subtract(cameraPosition, position, new Cartesian3())//c1 相机位置到模型位置向量
        ...
        const pn = Cartesian3.subtract(cameraPosition, cartesian3, new Cartesian3())//相机位置与     
        // 获取单位向量, 射线向投影向量投影
        ...
        Cartesian3.multiplyByScalar(pickRay.direction, -number1/number, cartesian3);//c5
        return cartesian3
    }
    ...
    getPlaneRotationPosition (position, cameraPosition, pickRay, axisDirection) {
        const cartesian3 = Cartesian3.subtract(cameraPosition, position, new Cartesian3())//c1
        const length = Cartesian3.dot(cartesian3, axisDirection)
        const number = Cartesian3.dot(pickRay.direction, axisDirection)
        Cartesian3.multiplyByScalar(pickRay.direction, -length / number, cartesian3);//c2
    ...
    extended (cartesian, length) {
        ...
        Cartesian3.add(cartesian, Cartesian3.multiplyByScalar(result, length, result), result)
        return result
    }
}
...
class Heatmap {
    ...
    constructor(options) {
        ...
        const material = this.createMaterial(points, heatmapOptions, heightRatio,width,height)
        const geometry = new THREE.PlaneGeometry(width, height, width*10, height*10)
    ...
    createMaterial(points, options, heightRatio,width,height) {
        const max = Math.max(...points.map(item => {return item.value}))
        const xMax = Math.max(...points.map(item => {return item.x}))
        const yMax = Math.max(...points.map(item => {return item.y}))

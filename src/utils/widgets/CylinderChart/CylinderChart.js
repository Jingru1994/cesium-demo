...
const d3 = Object.assign({}, require("d3-scale"), require("d3-geo"), require("d3-array"))
...
class CylinderChart {
    ...
    constructor(options) {
        ...
        for(let i = 0; i < data.length; i++){
            ...
            dataSeries.forEach(function(item) {
                dataDescription.push({ name: item.alias, value: data[i].properties[item.name], color: item.color })
                html += `<div style="height: 30px; line-height:30px;"><span style="color:${item.color}; font-size:20px">&#9679</span><span></span>${item.alias}：${data[i].properties[item.name]}</span></div>`
            ...
            const geometry = new THREE.CylinderGeometry(cylinderRadius,cylinderRadius,height,32)
            const cylinder = new THREE.Mesh(geometry,materials)
            cylinder.rotation.x = Math.PI/2
            cylinder.position.set(coordinate[0] + offset.x,coordinate[1] + offset.y, height/2 + offset.z)
            cylinder.html = html
            ...
            if(animate){
                const tween = new TWEEN.Tween({ scale: 0 })
                    ...
                    .onUpdate(({ scale }) => {
                        ...
                        cylinder.position.set(coordinate[0] + offset[0],coordinate[1] + offset[1], height/2*scale + offset[2])
                    })
                    .start()
            }
        }
    ...
    createMaterial(dataDescription) {
        ...
        const matetialBottom = new THREE.MeshPhongMaterial({color: dataDescription[dataDescription.length-1].color})
        const matetialTop = new THREE.MeshPhongMaterial({color: dataDescription[0].color})
    ...
    createPopup() {
        ...
        let popup = new PopupLegend(this._scene,this._camera,this._container,this._legendPanelColor)
        this._popup = popup
    ...
    onPointerMove(event,that) {
        ...
        if(intersects.length > 0) {
            if(!that._selectedObject || that._selectedObject !== intersects[0].object) {
                that._selectedObject = intersects[0].object
                that._popup.addTo(that._selectedObject).setHtml(that._selectedObject.html).setOffset(that._legendOffset)
            }
        }else {
    ...
    openPopup() {
        ...
        this._labelRenderer.domElement.addEventListener('mousemove',function _listener(e) {

...
class ParticleSystem {
  ...
  _createMaterial(params) {
    const diffuseTexture =
      params.options.diffuseTexture ||
      new THREE.TextureLoader().load("images/fire3.png");
  ...
  update(time) {
    this._material.uniforms.resolution.value = new THREE.Vector2(
      window.innerWidth,
      window.innerHeight
    );

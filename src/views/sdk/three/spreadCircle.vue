...
import SpreadCircle from "@/utils/sdk/Three/SpreadCircle/SpreadCircle.js";
...
export default {
  ...
  async mounted() {
    ...
    await this.createSpreadCircle();
  },
  beforeDestroy() {
    this.scene.clear();
    this.compositeScene.destroy();
    this.map.destroy();
    this.spreadCircle.destroy();
  },
  methods: {
    createScene() {
    ...
    async createSpreadCircle() {
      const projection = d3
        .geoMercator()
        .center([104.0, 37.5])
        .translate([0, 0])
        .reflectY(90);
      let point = [116.39929, 39.89301, 0];
      point = projection(point);
      let spreadCircle = new SpreadCircle({
        position: point,
        radius: 20,
        color: "rgb(204, 255, 0)"
      });
      this.spreadCircle = spreadCircle;
      spreadCircle.mesh.position.z = 2.1;

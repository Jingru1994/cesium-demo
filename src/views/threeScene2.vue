...
export default ({
    ...
    methods: {
            ...
            features.forEach((feature) => {
                feature.geometry.coordinates.forEach(coordinate => {
                    coordinate.forEach(points => {
                        ...
                        color = '#1A1A1A'
                        if(feature.properties.NAME === '东城区' || feature.properties.NAME === '西城区') {
                            color = '#D7221D'
                        }
        ...
        drawLine(posArr,color,z) {
            ...
            let lineMaterial = new LineMaterial({ color: new THREE.Color(color), linewidth:2.5, transparent: true, opacity: 0.7 })
        ...
        async createPlaneTexts() {
            ...
            let bj_a_point1 = await this.getData('data/beijing_around_point1.geojson')
            const that = this
            ...
            loader.load( 'threeFonts/Microsoft YaHei_Bold.json', function ( font ) {
                bj_point.forEach(function(item) {
                    ...
                    let position = that.projection(item.geometry.coordinates, that.center)
                    position = new THREE.Vector3(position[0], position[1],that.depth+0.05)

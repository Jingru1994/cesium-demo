...
export default ({
    ...
    methods: {
        ...
        pickObject() {
            ...
            let selectedObject
            this.labelRenderer.domElement.addEventListener('mousemove',onPointerMove)
            ...
            function onPointerClick() {
                if(selectedObject && selectedObject.name && selectObjects.includes(selectedObject.name)) {
        ...
        cameraAnimation() {
            ...
            const tween1 = new TWEEN.Tween(that.controls.target)
                .delay(3000)
                .to({ x: 1.0169825649746964, y: -0.6911753387875544, z: -0.4255155140638133 }, 3000)
                .easing(TWEEN.Easing.Cubic.InOut)
                .start()
                .onStart(function(){
                    new TWEEN.Tween(that.camera.position)
                        .to({ x:2.3106228148626595, y:0.01659965839722677, z:1.7890680865688264 }, 3000)
                        .easing(TWEEN.Easing.Cubic.InOut)
                        .start()
                    new TWEEN.Tween(that.camera.rotation)
                        .to({ x:-0.30933763162902417, y:0.5077586743913431, z:0.15416160517887822 }, 3000)
                        .easing(TWEEN.Easing.Cubic.InOut)
                        .start()
                })
            const tween2 = new TWEEN.Tween(that.controls.target)
                ...
                .onStart(function(){
                    new TWEEN.Tween(that.camera.position )
                        .to({ x:-0.16848833511087863, y:0.07267978846745511, z:-0.12717112239346018 }, 2000)
                        .easing(TWEEN.Easing.Cubic.InOut)
                        .start()
                })
            const tween3 =  new TWEEN.Tween(that.camera.position )
                .to({ x:-0.3132944104560116, y:0.08041758759218391, z:-0.054634510124828894 }, 1000)
                .easing(TWEEN.Easing.Cubic.InOut)
                .onComplete(()=> {
                    buttonLabel()
                })

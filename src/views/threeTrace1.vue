  ...
  methods: {
    ...
    mapClick(e) {
      ...
      if (this.selectedMode === "model") {
        if (sliderLine.style.width === "") {
          setTimeout(() => {
            sliderLine.style.width = 775 / 9 + "px";
          }, 100);
        }
        this.interval = setInterval(async () => {
          ...
          if (this.currentIndex === 9) {
            ...
            sliderLine.style.transition = "width 3s ease-in-out";
          }
          const width = this.currentIndex === 9 ? 0 : this.currentIndex + 1;
          sliderLine.style.width = (775 / 9) * width + "px";
        }, 3000);
      } else {
        if (this.interval) {
          clearInterval(this.interval);
    ...
    timeLineEnter(e) {
      ...
      this.sliderLine.style.width = (775 / 9) * this.currentIndex + "px";
      this.currentGrowthData = this.growthList[this.currentIndex];
      clearInterval(this.interval);
    },
    async timeLineLeave() {
      if (this.currentIndex !== 9) {
        this.sliderLine.style.transition = "width 3s ease-in-out";
        this.sliderLine.style.width =
          (775 / 9) * (this.currentIndex + 1) + "px";
        this.jump = false;
      } else {
        this.sliderLine.style.width = "0px";
        this.currentIndex = 0;
        setTimeout(() => {
          this.sliderLine.style.width = 775 / 9 + "px";
          this.sliderLine.style.transition = "width 3s ease-in-out";
        }, 500);
      ...
      this.interval = setInterval(async () => {
        ...
        const width = this.currentIndex === 9 ? 0 : this.currentIndex + 1;
        this.sliderLine.style.width = (775 / 9) * width + "px";
      }, 3000);
    }
  }
};
...
.trace-map-container {
  ...
  .slider-point {
    ...
    margin-right: calc(775px / 9 - 6.5px);
  }
  .slider-point.active {

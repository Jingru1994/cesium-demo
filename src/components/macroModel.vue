  ...
  methods: {
      ...
      this.clickHandler.setInputAction(function(movement) {
        ...
        if (Cesium.defined(pickedFeature)) {
          ...
          if (pickedFeature instanceof Cesium.Cesium3DTileFeature) {
            ...
            let infoTable = JSON.parse(
              pickedFeature.getProperty("jproperties")
            );
    ...
    createEntity(features) {
      ...
      for (let i = 0; i < features.length; i++) {
        ...
        let lablePosition = Cesium.Cartesian3.fromDegrees(
          longitude,
          latitude,
          200
        );
    ...
    async addPrimitivePolygon() {
      //以primitive方式加载geojson数据，primitive对象更底层，渲染效率更高，但是感觉他没有属性表
      let dataUrl = this.url;
      let geojsonData = await this.getData(dataUrl);
          ...
          new Cesium.GeometryInstance({
            ...
            attributes: {
              color: Cesium.ColorGeometryInstanceAttribute.fromColor(
                Cesium.Color.fromRandom({ alpha: 0.9 })
              )
            }
          })
        );

  ...
  methods: {
    addTiles() {
      ...
      if (this.classificationType) {
        options["classificationType"] = this.classificationType;
      }
      ...
      let panoPromise = Cesium.GeoJsonDataSource.load("/farm_wgs84.geojson", {
        //发起函数，异步写法
        stroke: Cesium.Color.fromCssColorString("#FFF173"),
        strokeWidth: 2,
        clampToGround: true
      }); // load完之后即为一个promise对象
      panoPromise.then(function(dataSource) {

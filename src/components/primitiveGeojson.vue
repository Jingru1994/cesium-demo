...
export default {
  ...
  methods: {
    ...
    addData() {
      ...
      const that = this;
      // debugger;
      this.geojsonPromise = Cesium.GeoJsonDataSource.load(this.url, {
        //发起函数，异步写法
        clampToGround: true,
        fill: new Cesium.Color(1, 1, 1, 0.01)
      }); // load完之后即为一个promise对象
      debugger;
      this.geojsonPromise.then(function(dataSource) {
        // 回调函数，异步读取json数据，数据读取成功后返回该对象（dataSource）,下面的功能因为是在数据加载成功后才有意义的，故放在同一个异步里
    ...
    addEvent() {
      ...
      this.clickHandler.setInputAction(function(movement) {
        ...
        if (
          ...
          let property = pickedFeature.id.properties.Name.getValue(
        ...
        if (
          ...
          if (!selected) {
            ...
            if (pickedFeature != selected) {
              selected.id.polygon.material.color = new Cesium.Color(
                1,
                1,
                1,
                0.01
